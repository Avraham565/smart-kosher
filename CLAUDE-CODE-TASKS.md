...
> ## מצב — עודכן 2026-08-18, ענף `fix/review-2026-08` (22 קומיטים, נדחף)
>
> **הסוויטה: 435 עוברים, 1 מדולג, 1,490 subtests** (בתחילת הסשן: 400 / 1,475).
...
>
> **10** (`ack_unjournaled`) · **11–13, 15, 17** (`zigbee_gateway`) · **16**
> (`api`/`views`) · **18–22** (קושחת H2) · **23** (hub/web/tools)
>
> **משימה 10 דורשת החלטה ארכיטקטונית** — איפה גר אוצר המילים של סטטוסי
> ה-executor (`executed` / `ack_unjournaled` / `failed`). היום הוא מחרוזות
> ב-`executor.py` ומשוכפל כ-`_JOURNALLED` ב-`scheduler.py:156`.
