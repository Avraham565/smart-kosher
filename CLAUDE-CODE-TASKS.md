...
> ## מצב — עודכן 2026-08-20, ענף `fix/review-2026-08` (51 קומיטים, נדחף)
>
> **הסוויטה: 488 עוברים, 1 מדולג, 1,512 subtests. קושחה: 1,247 בדיקות C.**
...
> | 12 | `wait_for_report` מבחין לפי endpoint | ✅ **חומרה** — מכשיר דו-גאנגי אמיתי: דיווח מגאנג 2 (`endpoint: 2`) **לא** אישר המתנה על גאנג 1, בזמן ש-`observed: True` כבר היה במצב. משימה 25 חיווטה |
> | 13 | `send()` לא זורק, `_pending` לא דולף | ✅ 3 טסטים נכשלו על הישן; 2 מוטציות |
> | 14 | `RepositoryValidationError` | היה בבסיס |
> | 15 | פינג לא מנטרל את הבּרֵקר | ✅ נכשל על הישן (`timeout` במקום `coordinator_down`) |
> | 16 | שני P2 בליבה | ✅ שניהם היו בלתי-נראים ל-468 טסטים |
> | 17 | שני P3 בליבה | ✅ 2 מוטציות |
> | 18 | טסט הצמדה לאוצר המילים של הפרוטוקול | ✅ נכשל מיד, תפס `missing_ieee` |
> | 19 | endpoint בהתאמת תגובות | ⚠️ **חוב מדידה** — `txn.c/h` מכוסים ב-1,247; חצי ה-`zb.c` **לא קומפל** |
> | 20 | פקיעת `bind` + verdict לא יומצא | ⚠️ **חוב מדידה** — כולו ב-`zb.c`, **לא קומפל**. טסט 18 האדים על `bind_no_response` |
> | 21 | `remove_device` יבדוק תוצאה | ⛔ **חסום — ראה למטה** |
> | 22 | גלישת דור בטבלת הטרנזקציות | ✅ test-first |
> | 23 | hub/web/tools (4 קומיטים) | ✅ טסט ההתנהגות מצא פער שלישי |
> | 24 | `ack_unjournaled` מקבל אישור | ✅ נכשל על הישן; 2 מוטציות |
> | 25 | אישור על ה-endpoint שפקדנו | ✅ **חומרה** — `confirm_target` החזיר `(ieee, 1)` מהרישום החי; חיובי וגם שלילי אומתו על הלוח |
>
...
>
> ### 26א — אומתה על חומרה, 2026-08-23
>
> `run_hwtest_gangs.py` על ה-DUT הדו-גאנגי, אותו ריג ואותן פקודות, רק
> ה-gateway מתחלף:
>
> ```
> 2410483^ (לפני)  on_off = True   endpoint_on_off = (אין)          exit 1
> HEAD     (אחרי)  on_off = False  endpoint_on_off = 1:False,2:True  exit 0
> ```
>
> בשתי הריצות `confirmed_by_device` על שתי הפקודות, כלומר המדידה תקפה. ובזכות
> ה-priming גאנג 1 **דיווח `False` בוודאות** — ולכן האדום מראה תא ש**נדרס**,
> לא תא שנשאר ריק. זה הבאג עצמו.
>
> ### נותרו — **21** (חסומה), **26ב** (אחרי 26א), **27**
>
> `cmd_remove_device` זורק את ערך ההחזרה של `ezb_zdo_nwk_mgmt_leave_req`,
> לא רושם callback, ומאשר `ok` ללא תנאי — בדיוק הדפוס שההערה ב-`zb.c` מתארת
> כבאג ההיסטורי. **הממצא אומת.** מה שלא ניתן לאמת הוא השאלה שהמשימה עצמה
> מתנה בה את הפתרון: האם ל-`ezb_zdo_nwk_mgmt_leave_req_t` יש שדה `cb`, והאם
> הפונקציה מחזירה `ezb_err_t`. **כותרות ה-SDK אינן בריפו** (אין
> `managed_components`, אין התקנת ESP-IDF), ו-`host_test` לא מקמפל את `zb.c`
> בכלל. המשימה מורה במפורש: *"אם הכותרת לא זמינה — אל תנחש. דווח ועבור
> הלאה."* — וכך נעשה.
>
> **ראיה נסיבתית בלבד:** כל קריאת `ezb_*` אחרת בקובץ נבדקת מול `EZB_ERR_NONE`,
> ולכן סביר שגם זו מחזירה `ezb_err_t`. סביר אינו מספיק כאן.
>
> **נבדק ונפסל כמקור:** יש עותק של `esp-zigbee-lib` על המכונה, בתוך ריפו
> דוגמאות של CrowPanel. הוא **`ESP_ZB_VER 1.6.4`** ולא 2.0.3, והוא במרחב
> השמות `esp_zb_*` ולא `ezb_*`. מה שיש בו הוא
> `void esp_zb_zdo_device_leave_req(param_t*, cb, ctx)` — **מחזיר `void`**
> ומקבל callback כארגומנט נפרד. מיפוי ממנו היה נותן בדיוק את התשובה ההפוכה
> לשאלה של משימה 21. **אל תשתמש בו.**
>
...
>
> ### אומת על חומרה — 2026-08-20
>
> `run_hwtest_ui.py` **66/66** · `run_hwtest_zmanim.py` **540 ערכים, סטייה
> מרבית 0.200 שנ׳ מול סבילות 2.0** · `run_hwtest.py` **27/27** (11, 13, 15, 17).
>
> **המדידה שסוגרת את 12+25**, על ה-DUT הדו-גאנגי `70:d0:7e:…` אחרי
> `enable_reporting` על endpoint 2:
>
> ```
> GANG2 on / waiting ep1 -> {'confirmed': False, 'observed': True} | reports: [(2, True)]
> GANG1 on / waiting ep1 -> {'confirmed': True,  'observed': True} | reports: [(1, True)]
> ```
>
> השורה הראשונה היא העיקר: הדיווח **הגיע**, `_states` **עודכן ל-True**, ובכל
> זאת ההמתנה על גאנג 1 סירבה. לפני משימה 12 זה היה מחזיר `confirmed: True`
> לפקודה שמעולם לא הגיעה — ו-`observed_state` נכנס ליומן. הרצה מוקדמת יותר
> החזירה `confirmed: False` עם `observed: None` ו-`reports: []`, כלומר סירוב
> מ"לא הגיע כלום" — **ירוק שאינו מוכיח דבר**, ולא נספר.
>
> ### ⚠️ מצב הספסל — ה-DUT השתנה, וזה מכוון
>
> `70:d0:7e:…` (`"מכשיר 2"`, היה `"גאנג אחד"`) **עבר `enable_reporting` על
> endpoint 2** ב-2026-08-20, כדי להוכיח את 12+25. ה-bind חי בטבלת ה-Zigbee של
> המכשיר עצמו ו**שורד אתחול** — הוא לא ייעלם מעצמו.
>
> **המשמעות: המכשיר הזה כרגע גרוע יותר מאחיו בשטח.** לפני הכתיבה גאנג 2 לא
> דיווח כלל ולכן לא יכול היה להשחית את מצבו של גאנג 1; מעכשיו כל שינוי בגאנג 2
> משחית אותו, עד ש-26א תיסגר. **יש עליו מנורה אמיתית ב-L2**, והתצוגה שלו
> משקרת.
>
> **אל תבטל את זה.** הוא כעת המכשיר היחיד שיכול להוכיח את 26א, כי הוא היחיד
> ששני הגאנגים שלו מדווחים בפועל. גם ה-short שלו התחלף ל-`0x3c20` בצימוד
> מחדש — אל תקבע short בקוד בדיקה, קרא מהרישום.
>
> ### חוב מדידה שהצטבר
>
> **בנייה אחת ב-ESP-IDF פותחת את 21 ומאמתת את 19+20.** `host_test` בונה
> `protocol.c` ו-`txn.c` בלבד — `zb.c` אינו מקומפל בשום מקום כאן, וההנחה
> ש-`rsp->in.header->src_ep` קיים על שני טיפוסי ההודעה של 19 היא ראיה חזקה
> (כל ארבעת ה-handlers קוראים מאותה כותרת) ולא קומפילציה.
>
> **נמדדו על החומרה, 2026-08-23** (`run_hwtest_ui`-style, לוח על COM8):
>
> 1. **`_save_registry` = 270–380ms, חציון ~350ms** (הראשון 600ms, קר).
>    **זו התשובה: כן, שווה לאחד.** הצטרפות מפעילה כמה שמירות ברצף —
>    `device_joined`, `device_endpoints`, `device_clusters` לכל endpoint,
>    ו-`reporting_configured` — כלומר צימוד של מכשיר דו-גאנגי הוא **מעל שנייה
>    וחצי של כתיבות פלאש חוסמות** על לולאה שיתופית אחת.
> 2. **`record()` + sync = 51–72ms, חציון ~69ms.** שבת עמוסה היא ~70ms לירייה.
>    **לא בעיה** — הבחירה "איטי מנצח לא-אמיתי" עומדת בעלות שנמדדה.
> 3. **הטיק הראשון = 304ms**, מול ~355ms שהבאפר קונה. ההנחה ב-`main.py` נכונה
>    לטיק. **2.6 השניות של הסנטינל לא שוחזרו** ואינן מוסברות על ידו
>    (`brain.create` = 840ms, גם הוא לא מספיק). הממצא האמיתי אחר: **הסלאק הוא
>    ספירת פריימים ולא זמן** — 3,166 בתים מתוך 4,096 (77%) משש תשובות ping,
>    כלומר הבאפר מחזיק 7–8 פריימים בגודל ping. ראה `docs/panel-update-model.md`.
>
> **הערה על הספסל:** ה-RTC של הלוח על `(2000,1,1)`, מתחת ל-`MIN_VALID_YEAR`,
> ולכן `tick()` חוזר מיד ולא יורה כלום — התנהגות נכונה לפי הכלל ב-`CLAUDE.md`,
> אבל היא הסיבה ששתי מדידות ראשונות החזירו `0ms`. מדידה של הטיק דורשת שעון
> מוזרק.
...
> **עודכן 2026-08-28: כתוב, מקומפל, צרוב, ואומת על החומרה.**
>
> `3f132ee` (קושחה) ו-`63b1465` (אדפטר) סגרו את שני החצאים ב-2026-08-23.
> **שאלת הכותרת נענתה בקריאה ולא בניחוש:** `ezb_zdo_nwk_mgmt_leave_req_t`
> נושא `cb` ו-`user_ctx`, והפונקציה מחזירה `ezb_err_t` — ולכן נבחר הענף
> העשיר: טרנזקציה, בלי ack בזמן שליחה, ו-delivered/failed מה-callback.
> הקומפילציה מוכיחה את שתי התשובות בנפרד: `leave.cb = …` ו-`!= EZB_ERR_NONE`
> לא היו עוברים אחרת. באותן בניות **19 ו-20 גם הן compile-verified**.
>
> **הצריבה כבר קרתה, וזה נמדד ולא הוסק.** `esptool verify-flash` מול
> `C:\tmp\h2_coordinator_flash` מחזיר digest תואם על שלושת האזורים —
> bootloader, טבלת מחיצות ואפליקציה. הבינרי הזה הוא הבנייה מ-23/8 20:11,
> והמקור שממנו נבנה זהה ל-`zb.c` שבעץ. השורה `Not flashed` בסוף `3f132ee`
> הייתה נכונה ב-20:12 והצריבה קרתה אחריה — **הודעת קומיט היא תצלום רגע,
> השבב הוא האמת.**
>
> אומת על החומרה 2026-08-28: `test_remove_device_answers_with_a_verdict`
> ב-`products/panel/hwtest/hwtest.py`. ‏`remove_device` לכתובת שאיש לא עונה
> בה **אינו** חוזר כהצלחה, בזמן שהקושחה הישנה הייתה מחזירה `ok` מיד.
>
> **מה שנשאר הוא לא 21:** פסק הדין חוזר אחרי 1511ms מול פקיעה של 8000ms
> בקושחה, כלומר האדפטר סוגר את החלון לפני שהקושחה עונה. נרשם כמשימה 60.
...
## משימה 26א — `_states` ו-`_expected` פר-(ieee, endpoint)

**חומרה: P1. תנאי מקדים ל-26ב — אל תבצע את 26ב לפניה.**

**קבצים בהיקף:** `src/smart_kosher/adapters/zigbee_gateway.py`,
`tests/test_zigbee_gateway.py`

**הרקע.** `_states` ממופתח לפי ieee בלבד (`:558`), וכך גם `_expected` (`:866`).
מכשיר דו-גאנגי הוא ieee אחד עם שני ממסרים, ולכן שניהם חולקים תא אחד — הדיווח
האחרון דורס את השני.

**הודגם על חומרה (2026-08-20)**, ה-DUT `70:d0:7e:…` אחרי `enable_reporting`
על endpoint 2:

```
GANG2 on / waiting ep1 -> {'confirmed': False, 'observed': True}
```

גאנג 1 היה **כבוי** באותו רגע. `observed: True` הוא מצבו של גאנג 2 שדרס את התא
שאמור לייצג את גאנג 1. הממתין סירב נכון — משימה 12 עובדת — אבל המצב עצמו כבר
היה משובש.

**שלוש השלכות, לא אחת.** רק הראשונה היא תצוגה:

| # | היכן | מה קורה |
|---|---|---|
| 1 | `devices():242` | ה-UI מציג ערך אחד לשני הגאנגים — של האחרון שדיווח |
| 2 | `_toggle:896` | `current = self._states.get(ieee)` — **toggle על גאנג 1 בוחר כיוון לפי גאנג 2, ושולח את הפקודה ההפוכה** |
| 3 | `:250` + `_expected` | כלל ההתערבות הידנית נשען על סטייה בין `_states` ל-`_expected`. בתא משותף, פעולה לגיטימית בגאנג 2 נראית כסטייה של גאנג 1 |

**מה לעשות.** מפתח `(ieee, endpoint)` לשני המילונים, ולכל קורא. `_on_state` כבר
מחזיק את ה-endpoint (`_state_ep`, משימה 12) — זה המקום שממנו הוא מגיע.
`confirm_target` (משימה 25) כבר יודע לאיזה endpoint פקדנו.

**שים לב לתאימות אחורה:** דיווח בלי endpoint (קושחה ישנה) חייב להמשיך לעבוד
כמו היום — אותו כלל של `_endpoint_admits`.

**אימות:** שלוש הפקודות. ואז [חומרה] — ה-DUT בספסל הוא היחיד שיכול להוכיח את
זה, כי שני הגאנגים שלו מדווחים בפועל.

---

## משימה 26ב — `_request_reporting` לכל endpoint ברשומה

**חומרה: P2. אחרי 26א בלבד.**

**קבצים בהיקף:** `src/smart_kosher/adapters/zigbee_gateway.py`,
`tests/test_zigbee_gateway.py`

**הרקע.** `_request_reporting:385` שולח `enable_reporting` עם
`entry.get("endpoint", 1)` — **endpoint אחד לכל מכשיר**. הרשומה כבר מחזיקה
`endpoints: [1, 2]` מתגלית ה-ZDO, ואיש לא קורא אותה כאן. גאנג 2 של מכשיר
דו-גאנגי לעולם אינו עובר bind, ולכן לחיצה ידנית עליו אינה מדווחת וכלל
ההתערבות הידנית אינו חל עליו.

**⛔ הסדר הוא העיקר, ולא סגנוני.** היום הפער של 26א **בלתי-נראה** רק משום
שה-bind לא נשלח: גאנג שלא מדווח אינו יכול לדרוס את המצב של שכנו. ברגע ש-26ב
נשלח לפני 26א, כל שינוי בגאנג 2 מתחיל להשחית את גאנג 1 — כלומר **מגלגלים את
הרגרסיה לכל מכשיר רב-גאנגי בכל בית**, ובכלל זה `_toggle` ששולח את הפקודה
ההפוכה.

**מה לעשות.** לולאה על `entry["endpoints"]`, דילוג על endpoints שאינם מכריזים
OnOff (cluster 6 — הרשומה כבר מחזיקה `clusters`). לשקול את חלון הקרדיטים
(`_REPORTING_WINDOW`): מכשיר בעל N endpoints צורך כעת N קרדיטים.

**אימות:** שלוש הפקודות. ואז [חומרה].
