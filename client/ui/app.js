...
async function renderConnection() {
  ...
  setMain(`
    <div class="settings-card">
      ...
      <div class="form-actions">
        <button class="btn-secondary" onclick="connectSerial()">התחבר ב-USB</button>
        <button class="btn-secondary" onclick="openConnectNetwork()">התחבר דרך הרשת…</button>
      </div>
    </div>
    ...
    <div class="settings-card">
      ...
      <div class="form-actions">
        <button class="btn-primary" onclick="submitProvision()">שמור ואתחל</button>
      </div>
    ...
    <div class="settings-card">
      ...
      <div class="form-actions">
        <button class="btn-primary" onclick="syncTime()">סנכרן שעון</button>
        <button class="btn-secondary" onclick="rebootDevice()">אתחול המכשיר</button>
      </div>
      <div id="time-msg"></div>
    </div>`);
}
...
function openConnectNetwork() {
  openModal('חיבור דרך הרשת', `
    ...
    <div class="form-actions">
      <button class="btn-secondary" onclick="closeModal()">ביטול</button>
      <button class="btn-primary" onclick="submitConnectNetwork()">התחבר</button>
    </div>`);
}
...
async function submitProvision() {
  ...
  setTimeout(() => { renderConnection(); updateHeaderStatus(); }, 8000);
}
...
async function rebootDevice() {
  if (!confirm('לאתחל את המכשיר?')) return;
  await api.post('/bridge/reboot', {});
  setTimeout(() => { renderConnection(); updateHeaderStatus(); }, 8000);
}
