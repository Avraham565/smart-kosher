...
1. `crowpanel_ui/` — early LVGL-on-CrowPanel exploration: a Hebrew/RTL rendering
   ...
   `products/panel/device/fonts/` carries its own byte-identical copies of
   those fonts, so nothing depended on this directory at the time it went.
...
3. [`products/panel/device/`](../products/panel/device/) — current.
   `display.py` is the direct descendant of `crowpanel_ui/hebrew_probe.py`
   (Hebrew/RTL/touch verified on the board 2026-07-16).
