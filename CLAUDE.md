...
ירוק כוזב **במנגנון האימות** גרוע מכל באג שהוא אמור לתפוס. ב-2026-08-17/18
המחלקה הזאת הופיעה כאן **חמש פעמים ביום אחד**. שתי צורות, ולכל אחת תרופה אחרת.

**א. המספר הוא נגזרת של קוד המדידה.** מבחן: אם אפשר לחזות את התוצאה מקריאת
הבדיקה בלי להריץ אותה — היא לא מדדה כלום.
- `hwtest_ui._capacity` החזיר בדיוק `declared × 2` לשלושה עמודים. זה היה חסם
  הלולאה: זריעת פריטים לעמוד **שמדפדף** מציירת שוב את אותו עמוד.
- ואז בדיוק `5` לשלוש גיאומטריות שונות. זה היה גובה הזכוכית — `_escapes` בודק
  יציאה מ-800×480, ושורה שגולשת מהמכל נוחתת **על** הרהיט שמתחתיה.
- מטא-טסט שהשווה `closure` מול `payload - {dropped}` בעצמו עבר על כל קלט,
  כולל payload ריק: הוא שכתב את אריתמטיקת הקבוצות במקום להפעיל את הבדיקה.
- `git show > file` ב-PowerShell הוסיף BOM, וארבעה טסטים נפלו על פרסינג במקום
  שניים על התוכן. ראיה **חזקה מהצפוי** היא סימן אזהרה, לא ניצחון.

**ב. המספר אמיתי, השאלה אחרת.** לפני שסופרים פלט של כלי כתשובה — לוודא על איזו
שאלה הוא באמת עונה.
- `Up to date` של mpremote אומר מה **לא נכתב**, לא מה **לא נעשה**. ממנו הוסק
  ש-`sync_core` כמעט חינם במצב יציב. המדידה: **20.9 / 20.7 / 20.9 שניות** —
  הוא עדיין הולך על כל החבילה ומשווה כל קובץ.

**הנוהל:** לפני הרצה — לכתוב מה מצופה ובאיזה טווח. תוצאה שיוצאת **נקייה מדי,
אחידה מדי או חזקה מדי** היא סיבה לבדוק את הכלי לפני שמעדכנים קוד לפיה. בכל
חמשת המקרים, קבלת התוצאה הייתה מחזירה באג שהרגע תוקן.
